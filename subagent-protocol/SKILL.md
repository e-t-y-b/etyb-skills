...
description: >
  Subagent coordination protocol for Claude Code's Agent tool -- dispatch focused agents for
  isolated tasks, run parallel agents for independent work, and perform two-stage review of
  subagent output. Manages the delegation mechanics (context packets, model selection, status
  signals, integration review) but NOT the orchestration decisions (that's the orchestrator).
  Use this skill whenever coordinating multi-agent work, dispatching focused subagents,
  running parallel agent tracks, reviewing subagent output, or designing agent context
  packets. Trigger when the user mentions "dispatch", "subagent", "sub-agent", "parallel",
  "delegate", "concurrent", "split work", "parallel tracks", "multi-task execution",
  "agent dispatch", "dispatch agent", "context packet", "agent context", "model selection",
  "haiku for", "sonnet for", "opus for", "parallel dispatch", "parallel agents",
  "two-stage review", "agent review", "subagent review", "integration review",
  "merge agent results", "agent coordination", "agent isolation", "context isolation",
  "context budget", "token budget", "dispatch plan", "agent pipeline", "orchestrated pipeline",
  "agent status", "DONE_WITH_CONCERNS", "NEEDS_CONTEXT", "BLOCKED", "dispatch overhead",
  "agent overhead", "re-dispatch", "agent iteration", "agent failure", "agent divergence",
  "narrow scope", "dispatch constraints", "concurrent agents", "independent tasks",
  "domain isolation", "file boundaries", "module boundaries", "service boundaries",
  "spec conformance", "quality review", "stage 1 review", "stage 2 review",
  or any question about how to split work across agents, what context to give an agent,
  which model to use for a subagent task, how to review agent output, how to run
  parallel agent work, or how to integrate results from multiple agents.
---
...
Subagent-specific verification checklist -- references `orchestrator/references/verification-protocol.md`.
...
When a dispatch cycle fails, follow the systematic debugging protocol from `orchestrator/references/debugging-protocol.md`: root cause first, one hypothesis at a time, verify before declaring fixed.
