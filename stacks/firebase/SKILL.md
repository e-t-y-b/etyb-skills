...
- **Per-product pages:** `stacks/firebase/<product>.md` — one per entry in `products_covered` above
- **Per-role views:** `stacks/firebase/<role>.md` — one per role in `applies_to_roles` above
