...
Per-product and per-role pages are maintained at **[docs.etyb.ai/stacks/anthropic-claude](https://docs.etyb.ai/stacks/anthropic-claude/)** with `last_verified_on` stamps and authoritative-source URLs. ETYB fetches from those URLs at runtime — see `skills/etyb/core/knowledge-currency.md` for the fetch contract.

- **Stack index:** <https://docs.etyb.ai/stacks/anthropic-claude/>
- **Per-product pages:** `https://docs.etyb.ai/stacks/anthropic-claude/<product>/`
- **Per-role views:** `https://docs.etyb.ai/stacks/anthropic-claude/<role>/` — composed views for each role in `applies_to_roles` above
