...
Per-product and per-role pages are maintained at **[docs.etyb.ai/stacks/gcp](https://docs.etyb.ai/stacks/gcp/)** with `last_verified_on` stamps and authoritative-source URLs. ETYB fetches from those URLs at runtime — see `skills/etyb/core/knowledge-currency.md` for the fetch contract.

- **Stack index:** <https://docs.etyb.ai/stacks/gcp/>
- **Per-product pages:** `https://docs.etyb.ai/stacks/gcp/<product>/`
- **Per-role views:** `https://docs.etyb.ai/stacks/gcp/<role>/` — composed views for each role in `applies_to_roles` above
