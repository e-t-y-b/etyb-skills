...
name: tdd-protocol
description: >
  TDD enforcement protocol that ensures test-driven development discipline during all code
  implementation. Enforces the red-green-refactor cycle on every code change — no production code
  without a failing test first, no exceptions. Provides deep knowledge of TDD mechanics,
  language-specific patterns, and evidence-based counters to common rationalizations for skipping
  tests. This is the execution discipline layer — qa-engineer defines test STRATEGY at plan time,
  tdd-protocol enforces DISCIPLINE during coding. Use this skill whenever implementation is
  happening and TDD discipline must be maintained. Trigger when the user mentions "TDD",
  "test-driven development", "test first", "test-first", "red green refactor", "red-green-refactor",
  "failing test", "write test before code", "write test first", "test before implementation",
  "test before code", "no code without test", "make the test pass", "minimal implementation",
  "green bar", "red bar", "refactor step", "TDD cycle", "test-driven", "outside-in TDD",
  "inside-out TDD", "London school TDD", "Chicago school TDD", "classical TDD", "mockist TDD",
  "spike and delete", "spike then TDD", "test-first development", "failing test first",
  "write a failing test", "make it green", "make it pass", "test-drive", "test-driving",
  "red phase", "green phase", "refactor phase", "commit rhythm", "small commits",
  "test-code-refactor", "I'll add tests later", "too simple to test", "no time for tests",
  "skip tests", "tests slow us down", "test after", "manual testing is enough",
  "prototype no tests", "test discipline", "TDD enforcement", "TDD protocol",
  or any situation where production code is being written and test discipline must be enforced.
  Also trigger when the user attempts to write implementation code without a corresponding
  failing test, rationalizes skipping tests, or needs guidance on the mechanical execution
  of the TDD cycle in any language or framework.
---
