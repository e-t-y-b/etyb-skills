...
description: Decision-memory and repo-map curator. Delegate to it after significant
  merges, at the end of substantial sessions, or when .etyb/memory/ needs maintenance
  — it refreshes the repo map (architecture, module ownership, test entry points,
  active plans), appends decision-log entries, compacts stale memory, and merges
  branch-scoped notes forward. The only agent that writes memory; its writes are
  confined to .etyb/memory/.
model: inherit
---
