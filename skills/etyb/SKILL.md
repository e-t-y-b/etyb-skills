...
name: etyb
description: >
  Engineering co-pilot for any software work — code, infrastructure, architecture, debugging, code review, deployment, testing, performance, security, planning, AI/ML, mobile, data, compliance. Use whenever the user describes a software situation OR asks for engineering help, even if they never say "engineering", "team", or "specialist". The situation itself is the trigger: "our Lambda times out at 6 s", "this race condition only repros in CI", "Android 14 crashes on launch", "audit this PR before I merge", "design the auth flow for our B2B SaaS with SCIM", "set up GitHub Actions for our Next.js app", "we're migrating off Heroku to AWS", "checkout p99 spiked 4x at 3am", "should I use Postgres or DynamoDB", "ECA migration deadline is May 11", "EHR sends HL7 v2 ADT — how do we map this to FHIR". A direct platform or technology mention is also sufficient — Kubernetes, Postgres, Redis, React, Next.js, Stripe, FHIR, Apex / LWC, Cloudflare Workers, Vercel, Vertex AI, Bedrock, anything in this category. The user does NOT have to ask for "help" or for a "team"; describing the situation is the trigger. Triggers: build me, design, architect, debug, fix this, why is this slow, why doesn't this work, race condition, intermittent failure, audit, review this, refactor, deploy, ship, set up, migrate, scale, harden, optimize, set up CI/CD, write tests, design this API, model this data, pick a database, plan this sprint, technical roadmap, modernize, re-platform, launch checklist, system audit, technical due diligence, production-ready, MVP, greenfield, end-to-end, full-stack, cross-team, how should I approach, what's the right way to build, plus any direct mention of a language, framework, cloud, vendor, or protocol (Postgres, React, Lambda, Kubernetes, Stripe webhook, FHIR, OAuth, etc.).
